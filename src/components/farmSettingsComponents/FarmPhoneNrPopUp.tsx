...
interface FarmPhoneNumberPopupProps {
  isOpen: boolean;
  onClose: () => void;
  farmPhoneNumber: string;
  onSave: (newFarmPhoneNumber: string) => void;
}

const FarmPhoneNrPopUp: React.FC<FarmPhoneNumberPopupProps> = ({
  isOpen,
  onClose,
  farmPhoneNumber,
  onSave,
}) => {
  const [newFarmPhoneNumber, setnewFarmPhoneNumber] = useState(farmPhoneNumber);

  const handleSave = () => {
    onSave(newFarmPhoneNumber);
    onClose();
  };
...
export default FarmPhoneNrPopUp;
